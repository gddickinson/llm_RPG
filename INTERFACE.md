...
- **`law.py`** — `LawSystem` (P12.9): per-settlement bounty ledger fed by break-ins/assault/robbery; adjacent guard + local bounty opens the 1-5 confrontation menu (pay / jail with skill-XP cost / bribe / one graded talk / resist); walking away shelves, never clears.
...
- **`hazards.py`** — P11.2: `flow_at` (rivers run along the longer water axis; lakes are slack), per-turn deep-water struggle (`water_hazard_tick`: fail → swept downstream + escalating drown damage; at 1 HP washed ashore minus one item), `tumble` off rock faces on bad climb fails; `[!]` telegraphs + hint-bar warning. P11.3 aids: carried gear (`equip_bonuses` climb/swim via `traversal.aid_bonus`), water_walking status skips checks and struggles, swimmers_grace +5, heavy-pack "drop or sink" telegraph.
