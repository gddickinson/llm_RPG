...
- **`battle_formation.py`** — P17.16 formations: `cohesion` (share of the squad facing the dominant way and clustered), `is_broken` (< 0.5), and the effects a LINE/LOOSE squad gets — LINE shield-overlap `defense_bonus` (+2 front-arc with a right-mate), `speed_mult` (½), `steady` (depth morale floor); LOOSE `incoming_ranged_mult` (½); `check_break` latches the one-time morale shock. Wired into `battle_ai.attack`/`update_morale` and `battle_session._steps`.
