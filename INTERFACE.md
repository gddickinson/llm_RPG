...
- **`battle_formation.py`** — P17.16/17 formations: `cohesion` (share of the squad facing the dominant way and clustered), `is_broken` (< 0.5), and per-formation effects — LINE shield-overlap `defense_bonus` (+2 front-arc with a right-mate), `speed_mult` (½ for LINE/RING), `steady` (LINE depth morale floor); LOOSE `incoming_ranged_mult` (½); the all-facing RING's `effective_arc` (every arc → front, the surround-counter), missile-vulnerability (×1.5) and `attack_penalty` (−2); `check_break` latches the one-time morale shock. Wired into `battle_ai.attack`/`_position_mods`/`update_morale` and `battle_session._steps`. P17.17 BRACE is `battle_ai._is_braced` (a `braced` flag or `order=="hold"`), gating `charge_attack`'s anti-cavalry hedge.
