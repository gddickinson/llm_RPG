...
- **`agent_sense.py`** — the away-agent's stateless sensing/eligibility predicates (2026-07-12b, split to hold the line): `_is_hostile`, `_colocated` (same-grid check — no phantom foes across coordinate spaces), `_healing_item`, `_knows_heal`, `_can_shoot` (needs matching ammo).
- **`away_mode.py`** — M.3 absent-player heartbeat: ticks the world on a slow cadence while the active hero's human is away (so the driving agent keeps it alive); any keypress hands back.
