...
- **`battle_session.py`** — P17.3 deterministic tick loop; `run_headless(max_ticks)` runs squads to a result (same shape as the resolver); P17.4c multi-step movement by unit `speed` (`MAX_STEPS` cap); P17.5 order-driven movement (`_order_move`: hold/retreat/goto/advance); P17.6b siege battering (engines hammer an adjacent wall to a breach before shooting); P17.6c capture-point victory (`_update_objectives`; `over`/`result` end on a seizure).
