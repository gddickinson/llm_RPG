...
- **`presence.py`** — P9A.7: NPCs within an enterable footprint are INDOORS — hidden from the street, shown inside as the same entities at zone-local spots; `npc_adjacent_to_player` is THE adjacency check (talk/hints/melee/barter). P14.2 magical sight: `sees_through_walls` (player has `keen_sight`) relaxes `hidden_by_walls` (the renderer's indoor-hiding rule) so you SEE behind walls — but reach (`npc_adjacent_to_player`) still stops at the stone.
