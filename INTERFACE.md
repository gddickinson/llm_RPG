...
- **`agent_nav.py`** — the away-agent's movement-safety layer (2026-07-12b, split from `agent_controller` to hold the line): pure functions over (engine, char) — `walkable` (zone-aware: the interior/dungeon floor when inside one, else the overworld with its wall-guard), `flee_step` (a walkable escape that opens distance, None when cornered), `safe_step` (step toward a goal but only onto a reachable tile — routes around walls instead of freezing), `zone_roam` (a random walkable in-zone tile). The guarantee that a driven hero never freezes.
