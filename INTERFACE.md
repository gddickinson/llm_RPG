...
- **`faction_ticker.py`** — Daily faction events; strength/stores drive encounters, shortages, rumors; repelled raids spawn a visible straggler. P17.8: brigand raids & guard patrols are settled by the real Lanchester resolver (`faction_battle.resolve_raid`) — the winner and the strength shed (`_casualty_hit`) come from the fight, not a die.
