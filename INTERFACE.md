...
- **`battle_ai.py`** — P17.3 group AI (ported colosseum brain): focus-fire target select, self-contained d20 strike, role movement (archers kite), squad morale/rout; P17.4c `step_toward` greedy push-into-contact when the centroid flow is blocked; P17.6b `nearest_struct` for siege engines homing on walls.
...
- **`battle_session.py`** — P17.3 deterministic tick loop; `run_headless(max_ticks)` runs squads to a result (same shape as the resolver); P17.4c multi-step movement by unit `speed` (`MAX_STEPS` cap); P17.5 order-driven movement (`_order_move`: hold/retreat/goto/advance); P17.6b siege battering (rams hammer an adjacent wall to a breach before shooting) + P17.6d ranged bombardment (artillery lobs at a wall from `SIEGE_RANGE`); P17.6c capture-point victory (`_update_objectives`; `over`/`result` end on a seizure).
