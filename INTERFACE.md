...
- **`faction_battle.py`** — P17.8 fold-back bridge: `army_for(faction, strength)` dresses a faction's abstract 0-100 strength as a `battle_resolve.Army` (bodies scale with strength, troop types by shares in `data/battles/faction_armies.json`), and `resolve_raid(...)` fights two factions through `resolve()` and returns the winner + survivor ratios. Lets off-screen faction battles use the real army model.
